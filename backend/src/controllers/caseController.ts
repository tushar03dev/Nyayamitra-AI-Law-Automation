...
export const getCaseById = async (
  ...
  try {
    ...
    if (
      caseData.organizationId.toString() !== req.user.organizationId.toString()
    ) {
...
// -------------------------------------------------------------
export const addCaseParticipant = async (
  req: AuthRequest,
  res: Response,
  next: NextFunction
): Promise<void> => {
...
// -------------------------------------------------------------
export const removeCaseParticipant = async (
  req: AuthRequest,
  res: Response,
  next: NextFunction
): Promise<void> => {
  ...
  if (!parsedParams.success) {
    res.status(400).json({
      errors: flattenZodError(parsedParams.error),
    });
    return;
  }

  if (!parsedUserParams.success) {
    res.status(400).json({
      errors: flattenZodError(parsedUserParams.error),
    });
    return;
  }
  ...
  try {
    ...
    caseData.assignedLawyers = caseData.assignedLawyers.filter(
      (id) => id.toString() !== req.params.userId
    );
    caseData.assignedJuniors = caseData.assignedJuniors.filter(
      (id) => id.toString() !== req.params.userId
    );
